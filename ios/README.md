...
ios/
  ...
  README.md                       this file
  OpenLink/
    Assets.xcassets/              AppIcon (generated by tools/make_ios_icon.py) + AccentColor
    Sources/
  ...
  OpenLinkTests/                  logic tests (pairing QR parsing)
```

Run them with ⌘U, or from a terminal:

```sh
xcodebuild test -project OpenLink.xcodeproj -scheme OpenLink \
  -destination 'platform=iOS Simulator,name=iPhone 16'
```

The test target compiles the handful of files under test directly rather than
hosting the app, so the suite needs no signing and nothing installed. It
covers the pairing QR, which is the one format this app and the Android app
have to agree on byte for byte — its fixtures are verbatim output of the
Kotlin builder, and a mismatch there is invisible to both compilers and
surfaces only as a scan that refuses on a real phone.
...
- **AppIcon** — the 1024x1024 PNG in `Assets.xcassets/AppIcon.appiconset` is
  generated, not hand-drawn: `python3 tools/make_ios_icon.py` renders it from
  the same shield path the Android launcher icon uses, on the same background,
  so both apps look like one product. Edit the script, not the PNG.
  `ASSETCATALOG_COMPILER_APPICON_NAME` in `project.yml` is what actually points
  the build at it; without that setting the catalog compiles and the app still
  installs with a blank icon.
